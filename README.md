...
graph TD
    ...
    N8N_Score -->|3. Phân tích nội dung| LLM[OpenAI GPT-4]
    ...
    N8N_Chat -->|8. Tổng hợp phản hồi| LLM
...
2. **Docker Desktop** (Để chạy nhanh cơ sở dữ liệu Vector Qdrant).
3. **Tài khoản n8n** (Tự cài đặt local bằng Docker/npm hoặc dùng n8n Cloud).
4. **OpenAI API Key** (Dùng để chạy mô hình ngôn ngữ lớn trích xuất và nhúng dữ liệu trong n8n).
...
### Bước 2.1: Chạy Vector Database Qdrant bằng Docker
Chạy lệnh sau trong Terminal để khởi động Qdrant trên cổng `6333`:
```bash
docker run -p 6333:6333 -p 6334:6334 -v qdrant_storage:/qdrant/storage qdrant/qdrant
```

> [!TIP]
> Bạn có thể kiểm tra xem Qdrant đã hoạt động chưa bằng cách truy cập bảng điều khiển đồ họa Dashboard tại: [http://localhost:6333/dashboard](http://localhost:6333/dashboard)
...
2. Lần lượt nhập các file từ thư mục `workflow/`:
   * **`CV Processing LLM Pipeline (Final).json`**: Nhận text CV và JD, gửi đến OpenAI để trích xuất cấu trúc ứng viên, phân tích điểm mạnh, điểm yếu và gửi vectors đến Qdrant.
   * **`Chatbot_Web_Fixed.json`**: Tiếp nhận tin nhắn từ giao diện Chat của Next.js, thực hiện tìm kiếm ngữ nghĩa trên Qdrant và trả về kết quả bằng AI.
